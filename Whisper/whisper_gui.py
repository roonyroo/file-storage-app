...
class WhisperGUI:
    ...
    def create_settings_frame(self):
        ...
        # Add always-on-top toggle checkbox
        self.always_on_top = tk.BooleanVar(value=True)
        on_top_check = ttk.Checkbutton(settings_frame, text="Always on top", variable=self.always_on_top, 
                                     command=self.toggle_always_on_top)
        on_top_check.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        
        # Help text
        help_text = "Press ' (single quote) to start recording, CAPS LOCK to stop"
        ttk.Label(settings_frame, text=help_text).grid(row=1, column=2, columnspan=2, padx=5, pady=5, sticky="w")
    ...
    def load_model(self):
        ...
        # No popup window, just update status bar
        ...
        try:
            ...
            try:
                ...
                # If direct loading fails, try explicit download
                logger.info(f"Downloading model: {model_id} (may take several minutes)")
    ...
    def on_key_press(self, event):
        # Start recording on single quote
    ...
    def toggle_always_on_top(self):
        """Toggle the window's always-on-top state based on checkbox"""
        is_on_top = self.always_on_top.get()
        self.root.attributes("-topmost", is_on_top)
        logger.info(f"Always-on-top set to: {is_on_top}")
